...
def inventoryIN(request):
    context = {'title': 'Inventory In'}
    return render(request, 'hackathon/inventoryIN.html', context)

##################
#  Inventory OUT  #
##################

def inventoryOUT(request):
    context = {'title': 'Inventory Out'}
    return render(request, 'hackathon/inventoryOUT.html', context)
...
def account(request):
    ...
    return render(request, 'hackathon/account.html', context)
...
def user_login(request):
    if request.method == 'POST':
        ...
        if user:
            if user.is_active:
                login(request, user)
                return HttpResponseRedirect('/hackathon/account/')
            else:
